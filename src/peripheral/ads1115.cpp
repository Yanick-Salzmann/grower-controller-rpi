...
namespace grower::peripheral {
    ...
    ads1115::ads1115(int32_t i2c_bus, int32_t device_id) {
        _device = i2c_open(pigpiod_handle(), i2c_bus, device_id, 0);
        if (_device < 0) {
            log->error("Error opening ADS1115 I2C device: {}", _device);
            throw std::runtime_error{"Error opening ADS1115 I2C device"};
        }
    ...
    uint16_t ads1115::read_single_channel(uint8_t channel) {
        ...
        i2c_write_word_data(pigpiod_handle(), _device, ADS1115_REG_POINTER_CONFIG, htons(config));

        std::this_thread::sleep_for(std::chrono::microseconds{_conversion_delay});

        const auto value = (uint16_t) i2c_read_word_data(pigpiod_handle(), _device, ADS1115_REG_POINTER_CONVERT);
        return htons(value);
    }
}
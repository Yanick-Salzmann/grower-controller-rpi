...
namespace grower::peripheral {
    tm1637_display::tm1637_display(uint8_t pin_clk, uint8_t pin_dio, uint32_t bit_delay) :
            ...
            _pin_clk{wiring::gpio::open_pin(pin_clk)},
            _pin_dio{wiring::gpio::open_pin(pin_dio)} {
    ...
    void tm1637_display::delay_bit() const {
        usleep(_bit_delay);
    }
    ...
    tm1637_display &tm1637_display::write_segments(const std::vector<uint8_t> &segments, uint8_t offset) {
        ...
        for (unsigned char segment : segments) {
            write_byte(segment);
        }

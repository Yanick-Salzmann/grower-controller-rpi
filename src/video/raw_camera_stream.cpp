...
namespace grower::video {
    LOGGER_IMPL(raw_camera_stream);

    void raw_camera_stream::initialize_camera(const camera_configuration &configuration) {
        ...
        status = mmal_port_enable(camera->control, &raw_camera_stream::camera_control_callback);
        if (status != MMAL_SUCCESS) {
        ...
        log->info("Actual video size: {}x{}", format->es->video.width, format->es->video.height);
        ...
        status = mmal_port_enable(video_port, &raw_camera_stream::video_data_callback);
        if (status != MMAL_SUCCESS) {
    ...
    uint32_t raw_camera_stream::map_format(VideoFormat format) {
        switch (format) {
            default:
    ...
    void raw_camera_stream::camera_control_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer) {
        ...
        auto *camera_stream = (raw_camera_stream *) port->userdata;
        camera_stream->handle_camera_control(port, buffer);
    }

    void raw_camera_stream::handle_camera_control(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer) {
        switch (buffer->cmd) {
    ...
    void raw_camera_stream::video_data_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer) {
        std::shared_ptr<MMAL_BUFFER_HEADER_T> p{buffer, [](auto *buff) { mmal_buffer_header_release(buff); }};
    ...
    void raw_camera_stream::handle_video_data(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer, std::shared_ptr<MMAL_BUFFER_HEADER_T> &buffer_ptr) {
        ...
        std::lock_guard<std::mutex> cl{_callback_lock};
        for (const auto &callback : _data_callback) {
            callback(_buffer_data, length);
        }
    }

    bool raw_camera_stream::start_capture() {
    ...
    void raw_camera_stream::stop_capture() {
    ...
    uint32_t raw_camera_stream::calculate_width(uint32_t width) {
        return VCOS_ALIGN_UP(width, 32u);
    }

    uint32_t raw_camera_stream::calculate_height(uint32_t height) {
